...
const getBannerSrc = (banner: string | null, file: File | null) => {
	return file ? globalThis.URL.createObjectURL(file) : banner ?? "";
};
...
const onBannerChange = (e: Event) => {
	const target = e.target as HTMLInputElement;
	editBannerFile.value = target.files?.[0] ?? null;
};
...
const saveEdit = async () => {
	...
	let bannerUrlToUpdate: string | undefined = undefined;
	...
	if (editBannerFile.value) {
		const uploaded = await uploadLogo(editBannerFile.value);
		if (uploaded) bannerUrlToUpdate = uploaded;
	}
...
<template>
	<div class="min-h-screen bg-linear-to-br from-slate-50 via-white to-slate-100 py-6 sm:py-10 px-4 sm:px-6">
			...
			<div class="bg-white/95 backdrop-blur rounded-3xl shadow-sm border border-slate-100 p-5 sm:p-8 lg:p-10">
				<div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
					<div>
						...
						<h1 class="text-2xl sm:text-3xl font-semibold text-slate-900 mb-1 sm:mb-2">Business Management</h1>
						<p class="text-sm sm:text-base text-slate-500">Manage your business profiles and review links</p>
					</div>
		...
		<div v-if="loading" class="max-w-6xl mx-auto">
			<div class="bg-white/95 backdrop-blur rounded-3xl shadow-sm border border-slate-100 p-8 sm:p-12 text-center">
		...
		<div v-else-if="!businesses.length" class="max-w-6xl mx-auto">
			<div class="bg-white/95 backdrop-blur rounded-3xl shadow-sm border border-slate-100 p-8 sm:p-12 text-center space-y-6">
			...
			<div class="grid gap-4 sm:gap-6">
				<div v-for="b in businesses" :key="b.id"
					...
					<div v-if="editingId === b.id" class="p-5 sm:p-7 lg:p-9">
						<div class="space-y-6">
							<!-- Logo Section -->
							<div class="flex flex-col sm:flex-row sm:items-start gap-4 sm:gap-6 pb-6 border-b border-slate-100">
							...
							<div class="pb-6 border-b border-slate-100">
								...
								<div class="space-y-4">
									<div v-if="b.displayBanner || editBannerFile" class="relative group">
										<img :src="getBannerSrc(b.displayBanner, editBannerFile)" alt="Banner"
											class="w-full h-48 object-cover rounded-2xl border border-slate-200 shadow-sm" />
										<div class="absolute inset-0 bg-black bg-opacity-40 rounded-2xl opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
											<span class="text-white text-sm font-medium">Change Banner</span>
										</div>
									</div>
									<div v-else class="w-full h-48 bg-linear-to-br from-gray-200 to-gray-300 rounded-2xl flex items-center justify-center">
					...
					<div v-else class="p-5 sm:p-7 lg:p-9">
						<div class="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-4 lg:gap-6">
							...
							<div class="flex items-start gap-3 sm:gap-4 lg:gap-6 flex-1 min-w-0">
								...
								<div class="flex-1 min-w-0">
									...
									<div class="space-y-1.5 sm:space-y-2">
										...
										<div v-if="b.displayBanner" class="pt-2">
											<span class="text-xs text-gray-500 font-medium block mb-2">Banner:</span>
											<img :src="b.displayBanner" alt="Banner" class="w-full max-w-xs h-24 object-cover rounded-xl border border-slate-200" />
										</div>
									</div>
								</div>
							</div>
							...
							<!-- Actions -->
							<div class="grid grid-cols-2 sm:grid-cols-4 lg:flex lg:flex-col gap-2 pt-4 lg:pt-0 border-t lg:border-t-0 border-slate-100 lg:border-none shrink-0">
								<button @click="startEdit(b)" class="btn btn-outline text-sm whitespace-nowrap justify-center">
									<svg class="w-4 h-4 sm:mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
										<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
									</svg>
									<span class="hidden sm:inline">Edit</span>
								</button>
								<button @click="() => copyLink(b.id)" class="btn btn-outline text-sm whitespace-nowrap justify-center">
									<svg class="w-4 h-4 sm:mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
								...
								<button @click="() => deleteBusiness(b.id)" class="btn btn-danger text-sm whitespace-nowrap justify-center">
									<svg class="w-4 h-4 sm:mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
